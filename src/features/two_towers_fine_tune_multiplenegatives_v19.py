...
checkpoints_dir = 'models/two_tower_checkpoints_multiplenegatives_v19'
...
class TwoTowerSimilarityModel(nn.Module):
    ...
    def forward(self, questions: List[str], paragraphs: List[List[str]]):
        # Embed the questions and paragraphs
...
class CustomMultipleNegativesRankingLoss(nn.Module):
    def __init__(self, model: TwoTowerSimilarityModel, scale: float = 10.0, similarity_fct=cos_sim):
...
optimizer = AdamW(two_tower_model.parameters(), lr=2e-5, weight_decay=0.01)
...
train_dataloader = DataLoader(train_dataset, batch_size=1, shuffle=True, collate_fn=collate_fn, num_workers=0)
val_dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False, collate_fn=collate_fn, num_workers=0)
