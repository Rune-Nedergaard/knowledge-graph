        ...
        for filename, paragraphs in zip(filenames, paragraphs_list):
            for index, paragraph in enumerate(paragraphs):
                if not embedding_file_exists(embeddings_folder, filename, index):
                    # Embed the paragraph using the BertBase model
                    _, paragraph_embedding, _ = model.embed_text(paragraph)
                    paragraph_embedding = paragraph_embedding.to(device)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('Embeddings saved to final_dabert_embeddings.h5')
    except Exception as e:
        print("Failed to save embeddings with h5py. Error:", e)

        try:
            with open('final_dabert_embeddings.pickle', 'wb') as f:
