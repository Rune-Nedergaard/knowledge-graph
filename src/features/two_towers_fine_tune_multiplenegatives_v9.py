...
checkpoints_dir = 'models/two_tower_checkpoints_multiplenegatives_v9'
...
for epoch in range(num_epochs):
    ...
    for batch in train_progress_bar:
        ...
        # Increment the step counter and save the model if necessary
        step_counter += 1
        if step_counter == num_steps_to_save:
            step_counter = 0
            save_path = os.path.join(checkpoints_dir, f"model_step_{step_counter}_epoch_{epoch+1}.pt")
            torch.save(two_tower_model, save_path)
            print(f"Model saved at step {step_counter} of epoch {epoch+1}")
