...
# Load the data
data = pd.read_csv('data/OF2019results.csv')

# Print the first five rows of the data
print(data.head())

# Extract the scores as a list
scores = data['TOTAL'].tolist()
...
plt.axvline(57, linestyle='-', color='mediumaquamarine', label='GPT-3.5',linewidth=2)
plt.axvline(72, linestyle='-', color='magenta', label='GPT-4',linewidth=2)
plt.axvline(68, linestyle='-', color='orange', label='QA, t=0.5 constrained', linewidth=2)
...
legend_elements = [plt.Line2D([0], [0], linestyle='-', color='mediumaquamarine', label='GPT-3.5'),
                   ...
                   plt.Line2D([0], [0], linestyle='-', color='orange', label='QA, t=0.5 constrained'),
